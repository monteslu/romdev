...
- `debug` — inspectSprites, inspectPalette, getCPUState (all 12), getAudioState (the 10 systems with a sound chip — all but Atari 2600/7800), getRenderingContext, findWriter (write watchpoint, all 12), **disassemble**/disassembleProject (11 — not GBA's ARM7), symbol lookup, whichTilesAreRendered, addressToSymbol, plus **cheats** (`gameCheats` = a free labeled RAM/code map for known ROMs, `applyCheat`/`clearCheats` non-destructively, `makeCheat` to create codes)
