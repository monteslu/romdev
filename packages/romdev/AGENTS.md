...
- **`findWriter({ address, maxFrames, pressDuring })` — the precise one (NES).**
  ...
  ```
  Supported on NES (fceumm), GB/GBC (gambatte), Genesis + SMS/GG (gpgx), and
  SNES (snes9x); other platforms return `notSupported` — use `watchMemory`
  there. On a banked mapper a `$8000-$BFFF` pc may be in a switchable bank —
  pass the right `bank` to `disassembleRom`.
