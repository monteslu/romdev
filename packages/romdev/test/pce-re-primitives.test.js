...
test("pce RE primitives: setRegister + watchRange + logPCRange (HuC6280)", { timeout: 200000 }, async () => {
  ...
  const build = toJSON(await client.callTool({ name: "build", arguments: { output: "rom",  platform: "pce", source: SRC } }, undefined, { timeout: 200000 }));
  ...
  const wr = toJSON(await client.callTool({ name: "watch", arguments: { on: "range",  start: 8192, end: 8192, kind: "write", frames: 10, limit: 10 } }));
  ...
  const cov = toJSON(await client.callTool({ name: "watch", arguments: { on: "pc",  start: 57344, end: 65535, frames: 10 } }));
