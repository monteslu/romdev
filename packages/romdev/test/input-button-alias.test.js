...
// aliases to the EMPIRICALLY-VERIFIED mapping (probed against the running
// genesis_plus_gx / gpgx core 2026-06-05 with an SGDK JOY_readJoypad probe).
//
...
test("SMS/GG '1'/'2' aliases resolve to libretro 'a'/'b'", () => {
  for (const plat of ["sms", "gg"]) {
    assert.equal(resolveButtonAlias("1", plat), "a", `${plat}: button 1 = libretro a`);
    assert.equal(resolveButtonAlias("2", plat), "b", `${plat}: button 2 = libretro b`);
  }
});
