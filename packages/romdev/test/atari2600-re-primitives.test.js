...
test("atari2600 RE primitives: setRegister + watchRange + logPCRange (6502)", { timeout: 200000 }, async () => {
  ...
  const wr = toJSON(await client.callTool({ name: "watch", arguments: { on: "range",  start: 128, end: 128, kind: "write", frames: 10, limit: 10 } }));
  ...
  const cov = toJSON(await client.callTool({ name: "watch", arguments: { on: "pc",  start: 61440, end: 65535, frames: 10 } }));
