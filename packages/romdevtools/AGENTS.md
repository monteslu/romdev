...
- **No battery save?** Many carts use passwords or no save (and Atari 2600/7800 + Lynx never had cartridge saves; C64 saves are disk-based — romdev loads/runs/distributes `.d64` disks, see the C64 platform notes, but a running game's own disk-save isn't persisted back out yet). `save_ram` is empty there and the tools say so plainly — use a full-machine savestate (`state({op:'save'/'load'})`) instead.
