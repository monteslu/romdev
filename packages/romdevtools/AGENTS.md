...
This doc is deliberately GENERIC — it can't hold 17 platforms' worth of detail without bloating every session. The knowledge that actually saves you — the memory map, the input/control quirks, the render-enable order, the codegen traps, the SDK's footguns — lives in each platform's docs, read on demand:
...
3. **Disassemble / trace** whenever the hack is about CODE or about data the
   ...
   Rizin/Ghidra analysis ops cover 17/17 platforms (incl. the 3D consoles' MIPS/SH-4). For a no-cheats game or a
...
> **the RE-INJECT trio** (put an edited asset BACK, all 14 classic platforms): **`romPatch({op:'findPointer'})`**
...
> work on every platform that has the register-write/watch core hooks (the 14 classic platforms).
> `disasm({target:'rom'})` + `disasm({target:'references'})` cover **all 17** (incl. the 3D consoles' MIPS/SH-4), and `disasm({target:'project'})` byte-exact reassembly the **14 classic platforms** — every
...
Key primitives (all bundled, all 14 classic tier-1 systems unless noted; full detail in the
playbook):
...
`disasm({target:'rom'|'references'})` covers **all 17 platforms** (incl. the 3D consoles'
MIPS R3000/R4300 + SH-4); byte-exact `disasm({target:'project'})` reassembly is the **14
classic platforms**. Every CPU
