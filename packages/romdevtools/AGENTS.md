...
This doc is deliberately GENERIC — it can't hold 19 platforms' worth of detail without bloating every session. The knowledge that actually saves you — the memory map, the input/control quirks, the render-enable order, the codegen traps, the SDK's footguns — lives in each platform's docs, read on demand:
...
**Co-driving is detected for you.** While the human is actively pressing (pad or keyboard), the window's input wins over yours and its real-time loop races your frame-stepping — and you'll KNOW: `frame`/`input` responses carry a `humanCoDriveWarning` while they pressed within the last ~2s, and `catalog({op:'status'})` / `playtest({op:'status'})` expose `humanInputActive`. When the human is idle the window leaves your `input({op:'set'})` alone. For deterministic stepping while they play, either `host({op:'pause'})` (the window keeps rendering, frozen) or use a SECOND session (a different `session` argument — or `x-romdev-session` header over HTTP — = a fully isolated emulator).
...
- `input` — drive controllers, look up hardware bit layouts. `input({op:'set', ports})` is POSITIONAL: `ports[N]` IS port N (there is no `port` key), and each button is its own boolean (`{a:true, b:true}`, never `{buttons:['a','b']}`). A malformed port object is REJECTED rather than partially applied — a press that silently doesn't happen turns into a false NEGATIVE about the game, which is the most expensive wrong answer this tool can give. `navigate` walks menus by advancing on SCREEN CHANGE (not fixed frames) and reports whether each press was consumed — the fast, reliable way to script a UI. **`input({op:'pointer', x, y, left?, right?, id?, active?})`** is absolute mouse/touch for wasmcart carts that declare FLAG_POINTER: `id` selects the pointer SLOT (0 = mouse, the default; 1-9 = touch fingers — several slots at once is a multi-finger gesture), and `active:false` releases a slot the way a real host does on touchend. Use `id:1+` to test the commonest cart-side portability trap: a cart that polls only pointer[0] works perfectly with a desktop mouse and silently ignores every touch on a phone. **`input({op:'wheel', notches})`** is the scroll wheel (wasmcart ABI v3.1): one click of a detented wheel is `notches:1`, fractions are allowed (`0.25` = a trackpad nudge), and POSITIVE IS UP — the SDL/LOVE convention, not the browser's downward deltaY. `notchesX` scrolls horizontally; `dx`/`dy` are the raw 1/120-notch units if you are asserting exact ABI values. The delta belongs to the NEXT frame stepped and is CLEARED after it, so a wheel call must be followed by a frame step to be observed, and repeated calls before that step ACCUMULATE the way a real trackpad flick does. A zero delta is REFUSED rather than accepted as a no-op — the host already clears the field every frame, so scrolling by nothing can only be a units mistake.
...
**Both protocol revisions are served on the same endpoint.** Legacy clients (`initialize` handshake + `Mcp-Session-Id`) work exactly as before. Modern clients speaking **2026-07-28** — no handshake, no session id, per-request `_meta` — are served statelessly, with `server/discover` advertising what this server supports. Since that revision has no protocol session to hold your identity, the server resolves it in this order: a `_meta["dev.romdev/sessionHandle"]` handle; **the optional `session` argument every tool accepts** — the one form you can produce from inside a tool call, so if you are on a modern client just pass one stable, descriptive slug (`session:"nes-platformer"`) on every call from the first `loadMedia` on; or, when a call names nothing, **your keep-alive connection** — the server keys the session to it and ends every result with a `session: <id>` line. Copy that id into `session:` if a later call ever says "No ROM loaded": a replaced connection starts empty, and the id is how you get your emulator back. A DIFFERENT `session` value is a fully isolated second emulator (deterministic stepping while a human plays in the first). Over plain HTTP the same job is done by the `x-romdev-session` header — send it on every call AND reuse one `Mcp-Session-Id`; re-initializing per call mints a fresh session even with the header set, which is how a second playtest window gets opened with the first orphaned.

**Hit a defect, or a job that cost far more calls than it should?** `feedback({op:'record', title, body, severity})` appends one stamped line (server version, pid, loaded platform + media, time) to a local, append-only log the human collects — nothing leaves the machine. One report per distinct issue; the server instructions call a host-toolchain install "a DEFECT to report", and this is where.
...
- **GameTank** — Clyde Shaffer's open-hardware W65C02S console (cc65 toolchain, ext `.gtr`). Full classic-style Tier-1: `cpu({op:'read'})` (65c02 regsnap), `audioDebug({op:'inspect', chip:'acp'})` (the ACP audio coprocessor), `breakpoint({on:'write'|'read'|'pc'})` + watchdog + coverage, `disasm` (`bytes`/`rom`/`references`/`project` + the 6502 Rizin/Ghidra cfg/xrefs/functions/decompile path — a 32KB flat cart @ `$8000`, one region, rebuilt byte-identical via `build({output:'reassemble', platform:'gametank', path})`), `cart({op:'extract'|'wrap'})`, and the 6502 re-inject path (`romPatch`) same as NES/C64/Lynx. **No `sprites({op:'inspect'})`/`background`** — it's a blitter framebuffer with no sprite OAM / no tilemap (like the Dreamcast). All five genre examples ship (`gametank/shmup|platformer|puzzle|sports|racing`) — real games on the bundled SDK draw-queue runtime, with pixel-art sprites loaded into GRAM. Fork with `examples({op:'fork', example, path, name})`, then `build({output:'project', path})`. `sports` is a genuine 2P game (the GameTank has two gamepad ports, unlike the handhelds).

- **sync32** — monteslu's RP2350 console (arm-none-eabi Cortex-M33, ext `.s32`). A cart is `game_main(api)` against a function-pointer ABI drawing into a flat 8-bit canvas: **no PPU, no tilemap, no OAM, no banking**. Build with `build({platform:'sync32', language:'c', sources, title, id, mode:'ram'|'xip'})` — the whole pipeline is WASM (cc1/as/ld/objcopy + a JS packer), no native gcc and no Python. A game with resources passes `data`/`dataPaths` and `api:2`; use **`form:'folder'`** if you want to RUN it here, because the core loads a bare executable plus a sibling `<name>/` data dir and does not unpack the `archive` tar you would ship. Inspectors: `memory({region:'system_ram'})` is the live 520KB SRAM at `0x20000000` (globals, stack, and in ram mode the code) so snapshot/diff/search/watchpoints all work; `cpu({op:'read'})` decodes r0-r15 + APSR + the FPv5-SP registers (**no `execPc`** — this is an interpreter, so r15 is already the next instruction); `palette({source:'live'})` reads the 256-entry RGB565 table; sprite sheets are the `sync32_sheet0` region (`0x1B0+n` for the rest). **No `sprites({op:'inspect'})`/`background`/`decompile`** — there is no OAM and no tilemap to decode, and no commercial ROMs to reverse-engineer. Docs: `platform({op:'doc', platform:'sync32', name:'mental_model'|'troubleshooting'})`. All five genre examples ship (`sync32/shmup|platformer|puzzle|sports|racing`) — fork one with `examples({op:'fork', example, path, name})`, then `build({output:'project', path})` in one call. **The idiom that surprises everyone:** `rect()`/`clear()` take an RGB565 colour but the canvas is 8-bit INDEXED, so the console snaps your colour to the NEAREST palette entry — a colour you did not put in the palette renders as something else.
