...
LogManager::LogManager() {
	...
	if (!consoleLog_) {
#ifdef __EMSCRIPTEN__
		consoleLog_ = nullptr;  // romdev: no stdout console (fd_write proxies to main + wedges on the app thread)
#else
...
void PrintfLog(const LogMessage &message) {
#ifdef __EMSCRIPTEN__
	(void)message; return;  // romdev: stdout proxies to main + wedges
#endif
	switch (message.level) {
