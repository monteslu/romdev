...
# lib the classic cores link) + a thin PS1 shim (ps1-debug.c → romdev_ps1_debug.c). The
# shim adapts the core's call sites to the shared hooks. Stage both into libpcsxcore,
# add to OBJS, hook the memory write/read paths + the interpreter step.
if [ ! -f libpcsxcore/romdev_ps1_debug.c ]; then
  RDBG_SRC="$(cd "$SCRIPT_DIR/romdev-debug" && pwd)"
  cp "$RDBG_SRC/romdev_debug.h" "$RDBG_SRC/romdev_debug.c" libpcsxcore/
  cp "$SCRIPT_DIR/patches/romdev-snippets/ps1-debug.c" libpcsxcore/romdev_ps1_debug.c
  # add both to OBJS
  sed -i 's|libpcsxcore/gte.o libpcsxcore/gte_nf.o libpcsxcore/gte_divider.o|libpcsxcore/gte.o libpcsxcore/gte_nf.o libpcsxcore/gte_divider.o libpcsxcore/romdev_debug.o libpcsxcore/romdev_ps1_debug.o|' Makefile
  # write hooks in psxmem.c (capture old byte, call romdev_ps1_write before each store)
  sed -i 's|#include "psxmem.h"|#include "psxmem.h"\nextern void romdev_ps1_write(unsigned int,unsigned int,unsigned int);\nextern void romdev_ps1_read(unsigned int);|' libpcsxcore/psxmem.c
  sed -i 's|\t\t\t\*(u8 \*)p = value;|\t\t\tromdev_ps1_write(mem, value, *(u8*)p);\n\t\t\t*(u8 *)p = value;|' libpcsxcore/psxmem.c
  sed -i 's|\t\t\t\*(u16 \*)p = SWAPu16(value);|\t\t\tromdev_ps1_write(mem, value, *(u16*)p);\n\t\t\t*(u16 *)p = SWAPu16(value);|' libpcsxcore/psxmem.c
  sed -i 's|\t\t\t\*(u32 \*)p = SWAPu32(value);|\t\t\tromdev_ps1_write(mem, value, *(u32*)p);\n\t\t\t*(u32 *)p = SWAPu32(value);|' libpcsxcore/psxmem.c
  # (the read-watch hook was historically a broken sed — the `|` in DebugCheckBP's
  # `| 0x80000000` collided with the s||| delimiter, and `\n` never matched a line —
  # so PS1 read-watch was never wired. Left unwired here too, parity with prior behavior.
  # romdev_ps1_read is defined but unreferenced; harmless. romdev_readwatch_* still exported.)
  # pc-break + coverage hooks in the interpreter step (set regs->stop on a hit; cov is
  # folded into romdev_ps1_step/on_dispatch now, so romdev_cov_mark is a kept no-op).
  sed -i 's|static inline void execI_(u8 \*\*memRLUT, psxRegisters \*regs) {|extern int romdev_ps1_step(unsigned int);\nextern void romdev_cov_mark(unsigned int);\nstatic inline void execI_(u8 **memRLUT, psxRegisters *regs) {\n\tromdev_cov_mark(regs->pc);\n\tif (romdev_ps1_step(regs->pc)) { regs->stop = 1; return; }|' libpcsxcore/psxinterpreter.c
