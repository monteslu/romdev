...
test("a plain loadMedia is completely unaffected",{ skip: TEST_ROM.skip || !HAVE_BEZEL }, async () => {
...
test("useActiveBezel with no sidecar fails loudly, naming the path it looked for",{ skip: TEST_ROM.skip || !HAVE_BEZEL }, async () => {
...
test("an explicit package loads, matches, and reports its regions",{ skip: TEST_ROM.skip || !HAVE_BEZEL }, async () => {
...
test("the composite is what a screenshot captures, and differs from the core frame",{ skip: TEST_ROM.skip || !HAVE_BEZEL }, async () => {
...
test("source:'both' returns the pair plus the three geometries", { skip: TEST_ROM.skip || !HAVE_BEZEL }, async () => {
...
test("source:'both' without a bezel says so instead of returning one picture twice", { skip: TEST_ROM.skip || !HAVE_BEZEL }, async () => {
...
test("catalog status advertises the running bezel",{ skip: TEST_ROM.skip || !HAVE_BEZEL }, async () => {
...
test("unloading media detaches the package",{ skip: TEST_ROM.skip || !HAVE_BEZEL }, async () => {
...
test("loading different media without a bezel drops the previous one",{ skip: TEST_ROM.skip || !HAVE_BEZEL }, async () => {
...
test("a reset KEEPS the package; a state load notifies it",{ skip: TEST_ROM.skip || !HAVE_BEZEL }, async () => {
