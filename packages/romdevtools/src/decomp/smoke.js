...
async function callTool(reg, name, args, sessionKey) {
  ...
  // runTool returns the MCP content envelope or a parsed object; normalize to an object.
 ...
 */
export async function runSmoke(project, { frames, inputs, sessionKey, sessionHandle }) {
  ...
  for (const side of sides) {
    ...
    results[side.name] = { load: { loaded: load?.loaded ?? null, core: load?.core ?? null }, png, width: img.width, height: img.height, pixelsSha1: createHash("sha1").update(img.data).digest("hex"), cpu, romSha1: await sha1File(side.rom), session: side.key, data: img.data };
  }
  ...
  const report = {
    project: project.id, frames, inputs: script.length, core: a.load.core ?? null, sessions: { original: a.session, rebuilt: b.session },
