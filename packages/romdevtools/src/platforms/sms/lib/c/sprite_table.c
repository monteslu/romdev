/* SMS sprite attribute table — shadow OAM in WRAM, DMA'd to SAT once
 * per frame.
 *
 ...
 *   $E0 instead: still off-screen (below the 192-line visible area on
 *   SMS) but NOT the terminator. Slots you don't touch stay invisible;
 *   slots you do touch render. Set a slot's Y to $D0 yourself if you
 *   ACTUALLY want the renderer to stop there.
 *
 * Workflow:
 *   sms_sprite_init();             // once
 *   sms_sprite_set(0, x, y, 4);    // game logic
 *   ...
 *   sms_sat_upload();              // once per vblank
 */
#include "sms_hw.h"
...
void sms_sprite_set(uint8_t slot, uint8_t x, uint8_t y, uint8_t tile) {
  shadow_oam[slot] = y;
  shadow_oam[0x80 + (uint16_t)slot * 2] = x;
  shadow_oam[0x80 + (uint16_t)slot * 2 + 1] = tile;
}

void sms_sat_upload(void) {
  uint8_t i;
  /* Y bytes → VRAM $3F00. */
  sms_vdp_set_addr(0x3F00, VDP_VRAM_WRITE);
  for (i = 0; i < 64; i++) PORT_VDP_DATA = shadow_oam[i];
  /* (X, tile) pairs → VRAM $3F80. */
  sms_vdp_set_addr(0x3F80, VDP_VRAM_WRITE);
