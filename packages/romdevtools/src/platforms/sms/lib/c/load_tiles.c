 ...
 *     sms_load_tiles(0x0000, my_tiles, sizeof(my_tiles));  // sprite bank
 *
...
void sms_load_tiles(uint16_t vram_dest, const uint8_t *src, uint16_t byte_count) {
  uint16_t i;
  sms_vdp_set_addr(vram_dest, VDP_VRAM_WRITE);
 ...
 */
void sms_set_tilemap_cell(uint8_t row, uint8_t col, uint8_t tile_idx, uint8_t attr) {
  uint16_t addr = 0x3800 + ((uint16_t)row * 32 + col) * 2;
  sms_vdp_set_addr(addr, VDP_VRAM_WRITE);
