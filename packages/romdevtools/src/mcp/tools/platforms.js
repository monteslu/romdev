...
const PLATFORM_QUIRKS = {
  ...
  ps1: {
    ...
    notes: [
      ...
      "WORKS NOW: run, frame({op:'screenshot'}), cpu({op:'read'}) (live R3000 registers), cheats, readMemory/writeMemory (system_ram = 2MB main RAM), and the static RE engine — disasm({platform:'ps1', target:'rom'|'functions'|'cfg'|'xrefs'}). MIPS is little-endian here.",
      "NOT YET: build (needs a PS1 toolchain like PSn00bSDK). The framebuffer renderer has no tile/sprite/nametable inspectors — use the screenshot + memory.",
  ...
  n64: {
    ...
    notes: [
      ...
      "WORKS NOW: run, frame({op:'screenshot'}) (real 3D frames, headless via FBO readback), cpu({op:'read'}) (live R4300 registers), cheats, readMemory/writeMemory (system_ram = 8MB RDRAM, 0x80xxxxxx maps to offset 0), and the static RE engine — disasm({platform:'n64', target:'rom'|'functions'|'cfg'|'xrefs'}). MIPS is big-endian here.",
      "NOT YET: build (needs libdragon as a WASM toolchain). 3D renderer has no tile/sprite inspectors — use screenshot + memory.",
