...
- **Matching decompilation (`decomp`, N64 proven)** — the function-level generate → compile → compare → refine loop against a registered splat project's OWN compiler and build (IDO via ido-static-recomp or MIPS GCC; the per-file compile command is captured from the project's `make`). `compare` compiles a candidate inside its real translation unit and returns ONE aggregate verdict — strict instruction + relocation equality vs the extracted asm, ROM-linked words vs the base ROM, the function's own jump tables/literals, and every other function in the TU unchanged; a check that could not run is `unknown`/`error`, never `exact`, and the verifier version is part of the result cache key. Around it: `import`/`resolve` (the splat segment map is the only VA→ROM resolver; overlay VAs are ambiguous until named), `plan`/`batch` (call graph from the built objects' relocations), `generate` (m2c with the TU's preprocessed context; `types propose:true` turns asm access widths into struct proposals fed back as `extraContext`/`declarations`), `search`/`job` (bounded decomp-permuter jobs with reports), `integrate`/`verify` (patch → full build → ROM sha1, auto-revert), `progress` (code bytes per object; asm, C, library, handwritten asm and retained inline asm kept apart), and runtime ops on a live session — `trace` (a real PC break at the function: a0-a3/f12/f14/stack at entry, v0/v1/f0 at return), `coverage` (instruction-exact from the core's PC bitmap, attributed to functions and basic blocks), `overlays`/`symbolize`, `state`, `smoke`. State lives in `~/.romdev/decomp/<project>/`; nothing touches the checkout except `integrate apply:true`. Backends pinned under `~/.romdev/tools/` (m2c, decomp-permuter). Doc: `platform({op:'doc', platform:'n64', name:'decomp'})`.
