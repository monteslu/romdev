# romdev-core-gambatte

gambatte — Game Boy / Game Boy Color emulator core (libretro), as WebAssembly.
...
**Built by:** `romdevtools/scripts/build-gambatte.sh` + patch(es) `gambatte-romdev-memory-regions.patch`. See `scripts/BUILD_MAP.md` in the romdev repo for the full recipe→package map.
